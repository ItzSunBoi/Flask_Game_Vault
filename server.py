...
PAGES = [
    ...
    'minesweeper', 'flapty', 'neonrun', 'connect4', 'uno', 'codenames', 'mafia', 'curvefever',
]
...
def remove_client_from_party(client_id, party_id=None):
    ...
    if party:
        party['members'].pop(client_id, None)
        if party.get('leader') == client_id:
            online = sorted(
                [cid for cid in party.get('members', {}) if clients.get(cid, {}).get('alive')],
                key=lambda cid: clients.get(cid, {}).get('created_at', 0)
            )
            all_members = sorted(list(party.get('members', {}).keys()), key=lambda cid: clients.get(cid, {}).get('created_at', 0))
            party['leader'] = (online[0] if online else (all_members[0] if all_members else None))
...
def get_party_payload(party_id):
    ...
    if party:
        for member in party.get('members', {}).values():
            ...
            members.append({
                ...
                'clientId': member.get('client_id'),
                'leader': party.get('leader') == member.get('client_id'),
    ...
    return {
        'type': 'party_state',
        'party': party_key,
        'leader': party.get('leader') if party else None,
        'members': members,
...
def join_party(client_id, party_id, name):
    ...
    # Remove stale duplicate by username in same party.
    for member_cid, member in list(party['members'].items()):
        if (member.get('name') or '').casefold() == player_name.casefold() and member_cid != client_id:
            party['members'].pop(member_cid, None)
            if member_cid in clients:
                clients[member_cid]['party'] = None

    party['members'][client_id] = {'name': player_name, 'client_id': client_id}
    if party.get('leader') not in party['members']:
        party['leader'] = client_id
    party['updated_at'] = time.time()
...
def build_game_context():
    return {
        'sync_connect4': lambda client_id, room, player: ws_send(client_id, {'type': 'c4_state', **get_connect4_state(room, client_id)}),
        'sync_uno': lambda client_id, room, player: ws_send(client_id, {'type': 'uno_state', **get_uno_view(room, client_id)}),
        'sync_codenames': lambda client_id, room, player: ws_send(client_id, {'type': 'codenames_state', **get_codenames_view(room, client_id)}),
        'sync_mafia': lambda client_id, room, player: ws_send(client_id, {'type': 'mafia_state', **get_mafia_view(room, client_id)}),
        'sync_curvefever': lambda client_id, room, player: ws_send(client_id, {'type': 'curve_state', **get_curve_view(room)}),
        'handle_drawing': handle_drawing_msg,
        'handle_wordgame': handle_word_msg,
        'handle_chess': handle_chess_msg,
        'handle_pong': handle_pong_msg,
        'handle_battleship': handle_battleship_msg,
        'handle_trivia': handle_trivia_msg,
        'handle_bomberman': handle_bomberman_msg,
        'handle_connect4': handle_connect4_msg,
        'handle_uno': handle_uno_msg,
        'handle_codenames': handle_codenames_msg,
        'handle_mafia': handle_mafia_msg,
        'handle_curvefever': handle_curve_msg,
        'start_drawing': start_drawing_game,
        'start_wordgame': start_word_game,
        'start_trivia': start_trivia,
        'start_bomberman': start_bomberman,
        'start_connect4': start_connect4,
        'start_uno': start_uno,
        'start_codenames': start_codenames,
        'start_mafia': start_mafia,
        'start_curvefever': start_curve,
    }
...
def sync_client_to_room(client_id, room, player):
    ...
    elif game in {'connect4', 'uno', 'codenames', 'mafia', 'curvefever'}:
        GAME_HANDLERS[game].sync(client_id, room, player, build_game_context())
    elif game == 'chess':
...
def create_room(room_key, game, public_room_id=None):
    return {
        ...
        # connect4
        'c4_board': [[0 for _ in range(7)] for _ in range(6)], 'c4_turn': 0, 'c4_winner': None,
        # uno
        'uno_state': None,
        # codenames
        'codename_state': None,
...
def leave_room(client_id):
    ...
    elif room['game'] in {'connect4', 'uno', 'codenames', 'mafia', 'curvefever'} and room.get('game_running') and len(room['players']) <= 1:
        room['game_running'] = False
        broadcast_all(room, {'type': 'game_over', 'winner': room['players'][0]['name'] if room['players'] else None})
...
def handle_message(client_id, msg, cookie_profile=None):
    ...
    if handler:
        handler.handle(client_id, room, msg, build_game_context())

    if t == 'start_game' and room['host'] == client_id and handler:
        handler.start(room, build_game_context())
...
# ── Connect 4 ─────────────────────────────────────────────────────────────────
def start_connect4(room):
    if len(room['players']) < 2:
        broadcast_all(room, {'type': 'waiting', 'message': 'Need 2 players.', 'canStart': True})
        return
    room['game_running'] = True
    room['finalized'] = False
    room['c4_board'] = [[0 for _ in range(7)] for _ in range(6)]
    room['c4_turn'] = 0
    room['c4_winner'] = None
    for i, p in enumerate(room['players'][:2]):
        p['c4_piece'] = i + 1
    broadcast_all(room, {'type': 'c4_state', **get_connect4_state(room)})

def get_connect4_state(room, client_id=None):
    return {'board': room.get('c4_board', []), 'turn': room.get('c4_turn', 0), 'players': get_player_list(room), 'winner': room.get('c4_winner')}

def c4_won(board, piece):
    for r in range(6):
        for c in range(7):
            if c <= 3 and all(board[r][c+i] == piece for i in range(4)): return True
            if r <= 2 and all(board[r+i][c] == piece for i in range(4)): return True
            if r <= 2 and c <= 3 and all(board[r+i][c+i] == piece for i in range(4)): return True
            if r >= 3 and c <= 3 and all(board[r-i][c+i] == piece for i in range(4)): return True
    return False

def handle_connect4_msg(client_id, room, msg):
    if msg.get('type') != 'c4_drop' or not room.get('game_running'):
        return
    player_idx = next((i for i, p in enumerate(room['players'][:2]) if p.get('client_id') == client_id), None)
    if player_idx is None or room.get('c4_turn', 0) != player_idx:
        return
    ...
    piece = player_idx + 1
    board[row][col] = piece
    if c4_won(board, piece):
        room['c4_winner'] = room['players'][player_idx]['name']
        room['game_running'] = False
    elif all(board[0][c] != 0 for c in range(7)):
        room['c4_winner'] = 'Draw'
        room['game_running'] = False
    else:
        room['c4_turn'] = 1 - room['c4_turn']
    broadcast_all(room, {'type': 'c4_state', **get_connect4_state(room)})

# ── UNO (lite) ────────────────────────────────────────────────────────────────
def _uno_make_deck():
    colors = ['R', 'G', 'B', 'Y']
    deck = [f'{c}{n}' for c in colors for n in range(10)] + [f'{c}{a}' for c in colors for a in ['S', 'R', 'D2'] for _ in range(2)] + ['W', 'W', 'W', 'W']
    random.shuffle(deck)
    return deck
...
def start_uno(room):
    if len(room['players']) < 2:
        return
    deck = _uno_make_deck()
    hands = {p['client_id']: [deck.pop() for _ in range(7)] for p in room['players']}
    top = deck.pop()
    room['uno_state'] = {'deck': deck, 'hands': hands, 'discard': [top], 'turn': 0, 'dir': 1}
    room['game_running'] = True
    for p in room['players']:
        ws_send(p['client_id'], {'type': 'uno_state', **get_uno_view(room, p['client_id'])})

def get_uno_view(room, client_id):
    st = room.get('uno_state') or {'hands': {}, 'discard': [], 'turn': 0}
    return {'players': [{'name': p['name'], 'count': len(st.get('hands', {}).get(p['client_id'], []))} for p in room['players']], 'top': (st.get('discard') or [None])[-1], 'turn': st.get('turn', 0), 'hand': st.get('hands', {}).get(client_id, [])}

def handle_uno_msg(client_id, room, msg):
    st = room.get('uno_state')
    if not st:
        return
    idx = next((i for i, p in enumerate(room['players']) if p['client_id'] == client_id), None)
    if idx is None or idx != st['turn']:
        return
    if msg.get('type') == 'uno_draw':
        if st['deck']:
            st['hands'][client_id].append(st['deck'].pop())
        st['turn'] = (st['turn'] + st['dir']) % len(room['players'])
    if msg.get('type') == 'uno_play':
        card = msg.get('card')
        hand = st['hands'].get(client_id, [])
        if card not in hand:
            return
        top = st['discard'][-1]
        if top != 'W' and card != 'W' and top[0] != card[0] and top[1:] != card[1:]:
            return
        hand.remove(card)
        ...
        if card.endswith('R'):
            ...
            nxt = (idx + st['dir']) % len(room['players'])
            for _ in range(2):
                if st['deck']:
                    st['hands'][room['players'][nxt]['client_id']].append(st['deck'].pop())
            step = 2
        if not hand:
            room['game_running'] = False
            broadcast_all(room, {'type': 'game_over', 'winner': room['players'][idx]['name']})
            return
        st['turn'] = (st['turn'] + step * st['dir']) % len(room['players'])
    for p in room['players']:
        ws_send(p['client_id'], {'type': 'uno_state', **get_uno_view(room, p['client_id'])})

# ── Codenames (lite) ─────────────────────────────────────────────────────────
def start_codenames(room):
    words = shuffle(['APPLE','CODE','MOON','RIVER','VAULT','LASER','DRAGON','SHIP','DESERT','EAGLE','ROBOT','SHADOW','NINJA','CLOCK','GLASS','MOUNTAIN','SPARK','GHOST','LIGHT','STONE','STORM','WHALE','HONEY','PAPER','METAL'])[:25]
    key = shuffle(['red'] * 9 + ['blue'] * 8 + ['neutral'] * 7 + ['assassin'])
    team = {p['client_id']: ('red' if i % 2 == 0 else 'blue') for i, p in enumerate(room['players'])}
    room['codename_state'] = {'words': words, 'key': key, 'revealed': [False]*25, 'turn': 'red', 'team': team}
    room['game_running'] = True
    for p in room['players']:
        ws_send(p['client_id'], {'type': 'codenames_state', **get_codenames_view(room, p['client_id'])})

def get_codenames_view(room, client_id):
    st = room.get('codename_state') or {}
    key = st.get('key', [])
    team = (st.get('team') or {}).get(client_id)
    show = team in ('red', 'blue')
    return {'words': st.get('words', []), 'revealed': st.get('revealed', []), 'key': key if show else [None]*len(key), 'turn': st.get('turn')}

def handle_codenames_msg(client_id, room, msg):
    st = room.get('codename_state')
    if not st or msg.get('type') != 'codename_guess':
        return
    idx = int(msg.get('index', -1))
    if not 0 <= idx < 25 or st['revealed'][idx]:
        return
    st['revealed'][idx] = True
    if st['key'][idx] == 'assassin':
        room['game_running'] = False
        winner = 'blue' if st['turn'] == 'red' else 'red'
        broadcast_all(room, {'type': 'game_over', 'winner': winner})
        return
    if st['key'][idx] != st['turn']:
        st['turn'] = 'blue' if st['turn'] == 'red' else 'red'
    for p in room['players']:
        ws_send(p['client_id'], {'type': 'codenames_state', **get_codenames_view(room, p['client_id'])})

# ── Mafia (lite) ──────────────────────────────────────────────────────────────
def start_mafia(room):
    if len(room['players']) < 4:
        return
    roles = ['werewolf', 'doctor', 'detective'] + ['villager'] * (len(room['players']) - 3)
    random.shuffle(roles)
    role_map = {p['client_id']: roles[i] for i, p in enumerate(room['players'])}
    room['mafia_state'] = {'phase': 'night', 'alive': {p['client_id']: True for p in room['players']}, 'roles': role_map, 'votes': {}}
    room['game_running'] = True
    for p in room['players']:
        ws_send(p['client_id'], {'type': 'mafia_state', **get_mafia_view(room, p['client_id'])})

def get_mafia_view(room, client_id):
    st = room.get('mafia_state') or {}
    return {'phase': st.get('phase'), 'alive': st.get('alive', {}), 'role': (st.get('roles') or {}).get(client_id), 'players': [{'id': p['client_id'], 'name': p['name']} for p in room.get('players', [])]}

def handle_mafia_msg(client_id, room, msg):
    st = room.get('mafia_state')
    if not st or msg.get('type') != 'mafia_vote' or not st['alive'].get(client_id):
        return
    target = msg.get('target')
    if target not in st['alive'] or not st['alive'][target]:
        return
    st['votes'][client_id] = target
    alive = [cid for cid,v in st['alive'].items() if v]
    if len(st['votes']) >= len(alive):
        counts = {}
        for t in st['votes'].values(): counts[t]=counts.get(t,0)+1
        eliminated = sorted(counts.items(), key=lambda kv: (-kv[1], kv[0]))[0][0]
        st['alive'][eliminated] = False
        st['votes'] = {}
        wolves = [cid for cid,r in st['roles'].items() if r=='werewolf' and st['alive'].get(cid)]
        others = [cid for cid,v in st['alive'].items() if v and st['roles'].get(cid)!='werewolf']
        if not wolves:
            room['game_running'] = False
            broadcast_all(room, {'type': 'game_over', 'winner': 'Villagers'})
            return
        if len(wolves) >= len(others):
            room['game_running'] = False
            broadcast_all(room, {'type': 'game_over', 'winner': 'Werewolves'})
            return
        st['phase'] = 'day' if st['phase'] == 'night' else 'night'
    for p in room['players']:
        ws_send(p['client_id'], {'type': 'mafia_state', **get_mafia_view(room, p['client_id'])})

# ── Curve Fever (lite) ────────────────────────────────────────────────────────
def start_curve(room):
    room['curve_state'] = {'players': {}, 'running': True, 'winner': None}
    for i, p in enumerate(room['players']):
        room['curve_state']['players'][p['client_id']] = {'x': 80 + i*40, 'y': 80 + i*40, 'a': 0.5, 'alive': True, 'trail': []}
    room['game_running'] = True
    broadcast_all(room, {'type': 'curve_state', **get_curve_view(room)})

def get_curve_view(room):
    return room.get('curve_state') or {'players': {}, 'running': False, 'winner': None}

def handle_curve_msg(client_id, room, msg):
    st = room.get('curve_state')
    if not st or not st.get('running') or msg.get('type') != 'curve_input':
        return
    pl = st['players'].get(client_id)
    if not pl or not pl['alive']:
        return
    turn = float(msg.get('turn', 0))
    pl['a'] += max(-0.3, min(0.3, turn))
    pl['x'] += 4 * math.cos(pl['a'])
    pl['y'] += 4 * math.sin(pl['a'])
    pl['trail'].append((round(pl['x']), round(pl['y'])))
    if not (0 <= pl['x'] <= 800 and 0 <= pl['y'] <= 500):
        pl['alive'] = False
    alive = [cid for cid, v in st['players'].items() if v['alive']]
    if len(alive) <= 1:
        st['running'] = False
        st['winner'] = room['players'][next((i for i,p in enumerate(room['players']) if p['client_id']==alive[0]),0)]['name'] if alive else None
    broadcast_all(room, {'type': 'curve_state', **get_curve_view(room)})
